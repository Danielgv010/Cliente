...
function listarEntrada() {
    ...
    salida.onsuccess = function () {
        ...
        for (persona of salida.result) {
            let fila = $("<tr>");
            let celdaNombre = $("<td>");
            celdaNombre.text(persona.nombre);

            let celdaApellidos = $("<td>");
            celdaApellidos.text(persona.apellidos);

            let celdaDni = $("<td>");
            celdaDni.text(persona.dni);

            let celdaContacto = $("<td>");
            celdaContacto.text(persona.contacto);

            let celdaFechaEntrada = $("<td>");
            celdaFechaEntrada.text(persona.fechaEntrada);
...
function listarSalida() {
    ...
    salida.onsuccess = function () {
        ...
        for (persona of salida.result) {
            ...
            celdaFechaSalida.text(persona.fechaSalida);
...
function buscarDniEntrada() {
    ...
    salida.onsuccess = function () {
        ...
        let fila = $("<tr>");
        let celdaNombre = $("<td>");
        celdaNombre.text(salida.result.nombre);

        let celdaApellidos = $("<td>");
        celdaApellidos.text(salida.result.apellidos);

        let celdaDni = $("<td>");
        celdaDni.text(salida.result.dni);

        let celdaContacto = $("<td>");
        celdaContacto.text(salida.result.contacto);

        let celdaFechaEntrada = $("<td>");
        celdaFechaEntrada.text(salida.result.fechaEntrada);

        let celdaMover = $("<td>");
        let botonMover = $("<button>");
        botonMover.attr("id", salida.result.dni)
        botonMover.click(function () {
            mover(this.id);
        });
        botonMover.text("Mover")
